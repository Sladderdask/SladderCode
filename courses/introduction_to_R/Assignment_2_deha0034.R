...
Point_distance <- function(p1,p2){
  return(Root_sum_squares(p2[1]-p1[1], p2[2]-p1[2]))
}
p1 <- c(1,3)
p2 <- c(4,4)
p3 <- c(41,24)
...
Point_distance <- function(p1 = c(3,0),p2 = c(0,4)){
  return(Root_sum_squares(p2[1]-p1[1], p2[2]-p1[2]))
}
#c)
...
Distance <- function(p1, p2){
  ...
  #Note that currently this function always creates an error.
  #Comment it out if you upload it to the server for testing other answers
...
m1 <- data.frame(x = c(2,0,12), y= c(0,3,0))
m2 <- data.frame(x = c(0,3,4), y= c(0,5,0))
...
# Example: say `m1` contains `v1`, `v2`, and `v3`, so `v1 = c(0,0)`,`v2 = c(0,3)`, and `v3 = c(12,0)` and
...
is_one_line <- function(p1,p2,p3){
  ...
  m <- rbind(p1,p2,p3) #you need to put something in place of ?. This currently creates an error
